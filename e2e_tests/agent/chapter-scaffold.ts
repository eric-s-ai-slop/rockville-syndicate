...
export function createChapterScaffold(index: number, slug: string, existingIds: readonly string[] = []): ChapterScaffold {
  ...
  if (existingIds.includes(chapterId)) throw new Error(`chapter id already registered: ${chapterId}`);
