...
const CHAPTERS: Array<{ title: string; classified?: boolean; sealed?: boolean }> = [
  { title: 'Maria Brooke' },
  ...
  { title: 'Rockville Syndicate: Origins', sealed: true },
...
for (const { title, classified, sealed } of CHAPTERS) {
  test(`smoke: ${title}`, async ({ page }) => {
    test.setTimeout(60000);
    await navigateToChapter(page, title, { classified, sealed });
